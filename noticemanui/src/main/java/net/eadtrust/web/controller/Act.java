...
public class Act {
   ...
   @RequestMapping(value = "/actresp", method = RequestMethod.POST)
   public ModelAndView act(HttpServletRequest request, HttpServletResponse response){
	   ModelAndView mav = new ModelAndView("/WEB-INF/view/result.jsp");
	   ...
	   HttpsURLConnection con = null;
		...
		KeyStore keyStore = KeyStore.getInstance("PKCS12", "BC");
		InputStream keyInput = SendAuth.class.getResourceAsStream("/eadtrust.p12");
		//keyStore.load(keyInput, ("7655e3af944830d31cd2").toCharArray());
		keyStore.load(keyInput, ("bb480af54138bfe870f9").toCharArray());
		keyInput.close();

		KeyManagerFactory kmf = KeyManagerFactory.getInstance("SunX509");
		//kmf.init(keyStore, ("7655e3af944830d31cd2").toCharArray());
		kmf.init(keyStore, ("bb480af54138bfe870f9").toCharArray());
		URL urlbis;
		
			urlbis = new URL("https://noticemanws.eadtrust.net/api_v1/act/"+request.getParameter("code"));
			con = (HttpsURLConnection) urlbis.openConnection();
			SSLContext context = SSLContext.getInstance("TLS");
			context.init(kmf.getKeyManagers(), null, new SecureRandom());
			con.setSSLSocketFactory(context.getSocketFactory());
			con.setRequestMethod("GET");
			con.setRequestProperty("Accept-Language","es,en-us;q=0.7,en;q=0.3");
			con.setRequestProperty("Connection","close");
		    InputStream is2 = con.getInputStream();
		    byte[] contentFile = IOUtils.toByteArray(is2);
		    System.out.println("El tamaño"+contentFile.length);
			response.setHeader("Content-Type","application/pdf; charset=UTF-8");
			response.setHeader("Content-Disposition", "filename=\"Acta\"");
			//DataOutputStream wr;
			//wr = new DataOutputStream (response.getOutputStream ());
			//wr.write (contentFile);
			//wr.flush ();
			//wr.close ();
		    ServletOutputStream responseOutputStream = response.getOutputStream();
		    responseOutputStream.write(contentFile);
		    responseOutputStream.flush();
		    responseOutputStream.close();

		} catch (Exception e) {
			mav.addObject("theModelKey", "<div class=\"error\">ERROR: Se ha producido un error desconocido al generar el acta, por favor revise si el CSV es correcto</div>");;
		}
	   return mav;
   }
    public ModelAndView handle(MultipartHttpServletRequest multipart, HttpServletRequest request) throws IOException, UnrecoverableKeyException, KeyManagementException, KeyStoreException, NoSuchProviderException, NoSuchAlgorithmException, CertificateException{
		...
		multipartObj = new MultipartUtility("http://localhost:8080/noticemanws/service/act", Charset.forName("UTF-8"), false);
